...
#define PLUGIN_UID 'Irbr'

#define WINDOW_WIDTH  500
#define WINDOW_HEIGHT 309
...
static void processReplacing(AEffect* effect, float** inputs, float** outputs, int32_t sampleFrames) {
    plugin_t* plug = (plugin_t*)effect->object;
    ...
    plug->r->process(sampleFrames, output, output);
}
...
void saveState(plugin_t* plug, void** data, int* size, int isBank) {
...
void loadState(plugin_t* plug, int size, int isBank) {
...
static intptr_t dispatcher(AEffect* effect, int32_t opCode, int32_t index, intptr_t value, void* ptr, float opt) {
    ...
    switch (opCode) {
        ...
        case effGetEffectName:
            strncpy((char*)ptr, "ImpulseLoader", VestigeMaxNameLen - 1);
